...
export default function Home() {
  ...
  useEffect(() => {
    fetchAll("homeContent").then((data) => setHero(data[0]));
    ...
    <div className="min-h-screen bg-surface">
      ...
      <section className="relative overflow-hidden bg-ink text-white">
        <video
          ...
          loop
          playsInline
          poster={hero?.heroPoster || heroImg}
          preload="metadata"
          className="absolute inset-0 h-full w-full object-cover opacity-30"
        >
          <source src={hero?.heroVideo || heroVideo} type="video/mp4" />
        </video>
        <div className="relative mx-auto max-w-4xl px-6 py-28 text-center">
          ...
          <h1 className="font-display text-4xl leading-tight tracking-wide lg:text-6xl">
            {hero?.heroTitle || "Bangladesh Industrial X-Ray"}
          </h1>
          <p className="mt-4 text-white/70">{hero?.heroSubtitle}</p>
          <Link
            to="/contact"
            className="mt-8 inline-block rounded-md bg-brand px-6 py-3 text-sm font-medium hover:bg-brand-dark"
          >
            Get a Quote
          </Link>
        </div>
      ...
      <section id="services" className="mx-auto max-w-6xl px-6 py-20">
        ...
        <div className="mt-10 text-center">
          <Link
            to="/services"
            className="rounded-md border border-brand px-6 py-2.5 text-sm font-medium text-brand hover:bg-brand hover:text-white"
          >
            View All Services
          </Link>
        </div>
